...
| L13 | a pre-0.2.0 store copy | point `EURYSX_DATA_DIR` at a v1 `eurysx.db`, run `report`, and compare JSON with the same run under 0.1.4 | `pragma user_version` becomes 2, `events` count unchanged, and every JSON key except `schema_version` and `aggregate_imports` is byte-identical |
