...
def main(argv=None):
    ...
    for agent, usages in agent_data.items():
        ...
        groups = preferences.budget_groups_for_usages(usages)
        if groups:
            stats.pacing = {}
            for policy_key, budget in groups.items():
                ...
                budget_usages = [usage for usage in budget_usages if usage.policy_key == policy_key]
                stats.pacing[policy_key] = UsageAnalyzer.pacing(stats, budget_usages, budget, datetime.now().date())
